...
| 35 | 8b/10b Line Encoder (Widmer / Franaszek) | the **physical-coding-sublayer (PCS)** workhorse between byte-domain logic and the SerDes of **PCIe Gen1/2 / SATA / 1000BASE-X GbE / USB 3.0 / DisplayPort / Fibre Channel** — maps each 8-bit character (+ 1-bit control flag `K`) to a **DC-balanced, transition-rich, comma-alignable 10-bit line code**: a tracked 1-bit **running disparity** picks the variant that pulls the 1s/0s balance back (no long-term DC bias ⇒ AC-coupling/PLL friendly), a guaranteed **≤ 5 consecutive identical bits** keeps the receiver **CDR** locked, and the control commas (K.28.1/.5/.7) embed the unique `0011111`/`1100000` pattern the deserializer uses for **byte alignment**; the **standout is datapath compactness** — 8b/10b splits the byte into a **5b/6b** and **3b/4b** sub-block each with two RD-selected representations, but **every RD+ codeword is the exact bitwise complement of its RD− partner** (for the disparity-neutral *and* the ±2 codes), so this core stores **only the RD− column** and selects with one XOR cone `emit=(RD==NEG)?cm:~cm`, the running disparity flipping **only on non-neutral sub-blocks** (`rd_next=neutral?rd:~rd`) ⇒ one 6-bit table + one 4-bit table + two XOR cones + two 1-bit flips, **no dual ROM, no adders in the disparity path**; correct **K.28 comma remap** (`001111` substituted for D.28's `001110` so the comma only ever appears in control codes) and the **Dx.A7 alternate 3b/4b** rule (primary `1110`/`0001` → alternate `0111`/`1000` when `RD=−1 & x∈{17,18,20}` or `RD=+1 & x∈{11,13,14}`, all control `.7` codes always alternate) that prevents a six-bit run at the sub-block boundary, **illegal-K detection** (`code_err_o`), **registered outputs ⇒ deterministic 1-clock byte→line latency** independent of data, `default_nettype none`/latch-free; independent **three-oracle** self-checking TB — (A) **table-free** structural laws on every symbol (popcount sub-block disparity legality, running disparity recomputed by popcount == `rd_o` and always ∈{−1,+1}, live max-run monitor asserting **≤ 5**), (B) **26 published known-answer anchors** (all 12 control codes + D.00, both RD states) asserted via `$fatal` **before** trusting the model, (C) golden-model scoreboard over a **full 256×2 data sweep** + all valid K codes + illegal-K flagging + a **6000-character randomized RD-chained stream** = **46,285 checks, 0 errors, max-run = 5** — waveform is a **real captured VCD** | [`Day35`](./Day35) |
| 36 | Hard-Decision Viterbi Decoder (rate-1/2, K=3 (7,5)) | the **maximum-likelihood decoder** for a convolutional code — the forward-error-correction workhorse behind **802.11a/g Wi-Fi, GSM, DVB, deep-space (CCSDS)** links: streams in one 2-bit channel symbol per clock and, after a fixed `TB_LEN`-symbol latency, emits one decoded message bit per clock, **correcting** channel errors rather than just detecting them by tracing the trellis path of minimum accumulated Hamming distance; **one trellis stage per cycle** — a **Branch-Metric Unit** (Hamming distance of the received symbol to every edge codeword, derived from the generator polys `G0=7₈`/`G1=5₈` at elaboration), four **Add-Compare-Select** butterflies (one per `2^(K-1)=4` state, each adding two incoming-edge metrics to its predecessors' path metrics, comparing, and pruning to a single survivor), **register-exchange survivor memory** (a `TB_LEN`-bit history word per state shifted with each winner's input bit so the decision falls out of the MSB — fixed latency, no separate traceback pass), **per-cycle metric normalisation** (subtract the running minimum ⇒ `PM_W` never overflows on unbounded streams), and reset-anchoring to state 0 + minimum-metric read-out so the decoder **self-synchronises** with no explicit trellis termination in hardware; parameterised generators/depth/metric-width, `default_nettype none`, latch-free; **self-checking TB** against a golden (7,5) **encoder** reference — random 400-bit streams, a **channel-error injection** test (one isolated flip per 25 symbols, all corrected since `d_free=5`), proper **zero-tail termination + drain**, full bit-exact alignment ⇒ **0 decoded-bit errors on every stream**; waveform is a **real captured VCD** showing the four path metrics settling and `out_valid` rising after the survivors fill | [`Day36`](./Day36) |
| 37 | Pipelined Radix-2 DIT FFT (N=16, Q1.15) | the **workhorse of DSP** — the transform under **OFDM (Wi-Fi 802.11a/g/n/ac, LTE/5G, DVB, DSL), pulse-Doppler & synthetic-aperture radar, spectrometry, fast convolution/correlation, channelizers** — a fully-parallel, fully-pipelined 16-point **Fast Fourier Transform** that ingests one *complete* 16-sample complex vector every clock and, after a fixed `LOG2N+1 = 5`-cycle latency, emits the *entire* 16-bin complex spectrum — **one transform per clock, back-to-back, no stalls**; **decimation-in-time (DIT)** radix-2 butterfly network fully **unrolled** into `LOG2N = 4` combinational stages each followed by a pipeline register bank, input consumed in **natural order** and internally **bit-reversed** (pure wiring) so the spectrum leaves in **natural bin order** `X[0..15]` with no external reorder on either side; the **twiddle ROM** `W_N^k = e^(−j2πk/N)` (`k = 0..N/2−1`) is **DERIVED AT ELABORATION** from `cos`/`sin` by a constant SystemVerilog function — **zero hand-typed tables** (like [Day 9](./Day9)/[Day 32](./Day32)); all arithmetic **fixed-point Q1.15** with **round-half-up** re-quantisation on the complex twiddle multiply (`wc·br−ws·bi`, `wc·bi+ws·br` `≫15`), and each butterfly **divides its sum/difference by two** so the four `/2` stages scale the result by exactly `1/N` — the output is the mathematically-exact **DFT ÷ N** (the **scaled-FFT** convention of commercial Xilinx/Intel streaming FFT IP) which keeps every internal node inside `[−1,1)` and makes the datapath **provably overflow-free for `|x| ≤ ½`**; the whole butterfly fabric is built with **`generate` loops whose bounds are all elaboration-time constants** (span/group/twiddle-stride are powers of two) ⇒ synthesizable structural unrolling with **no variable-bound procedural loops**, **deterministic outcome-independent 5-clock latency** (no data-dependent timing), streaming `in_valid`→`out_valid` with idle bubbles passing through unchanged (no stalls/no reorder), latch-free `default_nettype none`, parameterized `N`/`DW`/`LOG2N`; independent **direct-DFT golden model** self-checking TB (naive double-precision `O(N²)` double sum, structurally unlike the butterfly network, scaled `1/N` + Q1.15-rounded, graded within a **`TOL = 4` LSB** bound as any fixed-point FFT must be): directed corners (all-zero, DC, unit impulse, cosine tone @ bin 2, complex-exponential single-bin @ bin 5, two-tone, ±0.4 Nyquist, ramp) + **600 random streaming vectors** with idle bubbles ⇒ **19,456 checks, 0 mismatches, max observed error = 2 LSB** — waveform is a **real captured VCD** (DC→`X[0]=13107`, impulse→all bins `820`, tone→`X[2]=6554`, complex-exp→single peak at bin 5) | [`Day37`](./Day37) |
